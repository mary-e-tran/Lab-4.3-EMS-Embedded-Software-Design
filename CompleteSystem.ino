...
Adafruit_GC9A01A tft(TFT_CS, TFT_DC, TFT_SDA, TFT_SCL, TFT_RES);
...
void updateDisplay() {
  ...
  if (currentState == STP || currentState == WLK) {
    ...
    if (currentState == CAL) { //Replace once correct callibration code is created
      ...
      // Set to clear, small text size for data alignment
      tft.setTextSize(1);
      
      // Hardcoded mock values based on your visual mockup
      float expected_x = 1.08;
      float expected_y = 0.02;
      float expected_z = -0.23;
      
      float returned_x = 1.43;
      float returned_y = 0.92;
      float returned_z = -1.32;
      
      // Hardcoded result evaluation layout
      String testResult = "CALIBRATION FAILED"; 
      uint16_t resultColor = GC9A01A_RED; // Set to RED for failure, change to GC9A01A_GREEN if successful
      uint16_t lineEvColor = tft.color565(255, 255, 0); // Yellow lines from mockup
      
      // --- DRAW DIVIDING LINES ---
      // Vertical line: Starts above center, drops down to the horizontal line divide
      tft.drawFastVLine(cx, cy - 40, 55, lineEvColor);
      
      // Horizontal line: Splits the upper columns from the bottom text block
      tft.drawFastHLine(cx - 75, cy + 15, 150, lineEvColor);
      
      // --- LEFT COLUMN: EXPECTED VALUES ---
      tft.setTextColor(GC9A01A_WHITE);
      // Header
      tft.setCursor(cx - 85, cy - 35);
      tft.print("Expected values:");
      // Values
      tft.setCursor(cx - 70, cy - 15);
      tft.print("X: "); tft.print(expected_x); tft.print("g");
      tft.setCursor(cx - 70, cy - 2);
      tft.print("Y: "); tft.print(expected_y); tft.print("g");
      tft.setCursor(cx - 70, cy + 11);
      tft.print("Z: "); tft.print(expected_z); tft.print("g");
      
      // --- RIGHT COLUMN: RETURNED VALUES ---
      // Header
      tft.setCursor(cx + 10, cy - 35);
      tft.print("Returned values:");
      // Values (Using x_g_value, y_g_value, z_g_value variables)
      tft.setCursor(cx + 25, cy - 15);
      tft.print("X: "); tft.print(x_g_value); tft.print("g");
      tft.setCursor(cx + 25, cy - 2);
      tft.print("Y: "); tft.print(y_g_value); tft.print("g");
      tft.setCursor(cx + 25, cy + 11);
      tft.print("Z: "); tft.print(z_g_value); tft.print("g");
      
      // --- BOTTOM ROW: CALIBRATION STATUS ---
      tft.setTextSize(1); // Keep size 1 or change to 2 if you want it larger
      tft.setTextColor(resultColor);
      
      // Centering math: String length * 6 pixels wide per character at size 1
      int16_t resultWidth = testResult.length() * 6;
      tft.setCursor(cx - (resultWidth / 2), cy + 32);
      tft.print(testResult);
    }
  }
  unsigned long currentMillis = millis();
